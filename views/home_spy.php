    ...
    <tbody>
        ...
        <?php foreach ($favorites as $v) : ?>
            <tr>
                ...
                <td>
                    <input class="og-button og-button-little" type='button' value='<?= $lang['HOME_SPY_SEE'] ?>' onclick="window.open('index.php?action=show_reportspy&amp;galaxy=<?= $v['spy_galaxy'] ?>&amp;system=<?= $v['spy_system'] ?>&amp;row=<?= $v['spy_row'] ?>&amp;spy_id=<?= $v['spy_id'] ?>','_blank','width=640, height=480, toolbar=0, location=0, directories=0, status=0, scrollbars=1, resizable=1, copyhistory=0, menuBar=0');return(false)" />
                </td>
                <td>
                    <!-- todo del_favorite_spy ne semble pas fonctionner -->
                    <input class="og-button og-button-danger og-button-little" type='button' value='<?= $lang['HOME_SPY_FAVDELETE'] ?>' onclick="window.location = 'index.php?action=del_favorite_spy&amp;spy_id=<?= $v['spy_id'] ?>&amp;info=1';" />
                </td>
