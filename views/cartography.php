...
<?php
    for ($i = 1; $i <= $nb_colonnes_ally; $i++) {
        echo '<td class="c" colspan="2" style="width:300px; color:' . $color_ally[$i - 1] . ';">' . $lang['CARTO_ALLY'] . $i . "</td>\n";
    }
?>
        </tr>
        <tr>
<?php
    for ($i = 1; $i <= $nb_colonnes_ally; $i++) {
        echo '<th style="width:125px"><input type="text" name="select_ally_[' . $i . ']" onkeyup="autoComplete(this,this.form.elements[\'ally_[' . $i . ']\'],\'text\',false)"></th>' . "\n";
        echo '<th style="width:175px"><select name="ally_[' . $i . ']">' . $options_[$i] . "</select></th>\n";
    }
?>
        </tr>
        <tr>
            <td class="c" colspan="<?php echo $nb_colonnes_ally * 2; ?>"><input type="submit" value="<?php echo($lang['CARTO_DISPLAYPOSITIONS']); ?>">
            </td>
        </tr>
    </table>
</form>
...
<?php
        ...
        for ($i = $galaxy_down; $i < $galaxy_up; $i++) {
            echo '<td class="c" style="width:60px" colspan=' . $nb_colonnes_ally . '>';
...
<?php
        for ($system = 1; $system <= intval($server_config['num_of_systems']); $system = $system + $step) {
            ...
            echo "<tr>\n";
            echo "\t" . '<td class="c" style="white-space:nowrap">' . $system . " - " . $up . "</td>";
            for ($galaxy = $galaxy_down; $galaxy < $galaxy_up; $galaxy++) {
                ...
                foreach ($position as $ally_name) {
                    if ($galaxy_ally_position[$ally_name][$galaxy][$system]['planet'] > 0) {
                        $tooltip[$i] = '<table style="width:200px">';
                        $tooltip[$i] .= '<tr><td class="c" colspan="2">' . $lang['CARTO_PLAYER_POSITIONS'] . '</td></tr>';
                        $last_player = '';
                        foreach ($galaxy_ally_position[$ally_name][$galaxy][$system]['population'] as $value) {
                            ...
                            $tooltip[$i] .= '<tr><td class="c">' . $player . '</td><th>' . $row . '</th></tr>';
                ...
                for ($i = 1; $i <= $nb_colonnes_ally; $i++) {
                    echo "\t" . '<th style="width:20px"><a style="cursor:pointer"' . $tooltip[$i - 1] . '><span style="color:' . $color_ally[$i - 1] . '">' . $nb_player[$i - 1] . '</span></a></th>' . "\n";
                }
            }
            echo "\t" . '<td class="c" style="white-space:nowrap">' . $system . ' - ' . $up . '</td>';
            echo "</tr>\n";
