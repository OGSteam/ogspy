...
else $log = array($lang['ADMIN_LOGS_NOLOGS']);
    ...
    for ($i = 0; $i < 12; $i++) {
        ...
        if ($show == $show_year . "~" . $show_month . "~" . $show_day) {
            if (log_check_exist(date("ym", $date))) {
                ...
                echo "\t" . "<th width='40px'><input type='image' src='theme/images/save.png' onclick=\"window.location = 'index.php?action=extractor&amp;date=" . date("ym", $date) . "'\" title='".$lang['ADMIN_LOGS_DOWNLOAD']. strftime("%B %Y", $date) . "'><input type='image' src='theme/images/drop.png' onclick=\"window.location = 'index.php?action=remove&amp;date=" . date("ym", $date) . $show_day . "&directory=TRUE'\" title='".$lang['ADMIN_LOGS_DELETE'] . strftime("%B %Y", $date) . "'>";
            } else {
            ...
            if (log_check_exist(date("ym", $date))) {
                ...
                echo "\t" . "<th width='16px'><input type='image' src='theme/images/save.png' onclick=\"window.location = 'index.php?action=extractor&amp;date=" . date("ym", $date) . "'\" title='".$lang['ADMIN_LOGS_DOWNLOAD']. strftime("%B %Y", $date) . "'></th>";
            } else {
    ...
    for ($i = 1; $i <= $max_day; $i++) {
        ...
        if ($show == $show_year . "~" . $show_month . "~" . $show_day) {
            ...
            if (log_check_exist($show_year . $show_month . $day)) {
                echo "\t" . "<th><a>" . $day . "</a></th>";
                echo "\t" . "<th width='40px'><input type='image' src='theme/images/save.png' onclick=\"window.location = 'index.php?action=extractor&amp;date=" . $show_year . $show_month . $day . "'\" title='".$lang['ADMIN_LOGS_DOWNLOAD']. strftime("%d %B %Y", $date) . "'><input type='image' src='theme/images/drop.png' onclick=\"window.location = 'index.php?action=remove&amp;date=" . $show_year . $show_month . $day . "'\" title='".$lang['ADMIN_LOGS_DELETE']. strftime("%d %B %Y", $date) . "'>";
            ...
            if (log_check_exist($show_year . $show_month . $day)) {
                ...
                echo "\t" . "<th width='16px'><input type='image' src='theme/images/save.png' onclick=\"window.location = 'index.php?action=extractor&amp;date=" . $show_year . $show_month . $day . "'\" title='".$lang['ADMIN_LOGS_DOWNLOAD']. strftime("%d %B %Y", $date) . "'>";
            } else {
                echo "\t" . "<th colspan='2' onclick=\"window.location='index.php?action=administration&amp;subaction=viewer&amp;show=" . $show . "&amp;typelog=" . $typelog . "';\">";
                echo "<a style='cursor:pointer'><span style=\"color: lime; \">" . $day . "</span></a></th>";
            }
