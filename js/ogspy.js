...
function ogspy_beginCreateUser() { 
	_adminVisible('createNewPlayer');
	_adminUnvisible('creatingNewPlayer');
}
function ogspy_endCreateUser() { 
	_adminVisible('creatingNewPlayer');
	_adminUnvisible('createNewPlayer');
}
// views/profile.php
function ogspy_checkPassword(form, message) { 
	...
	if (oldPassword !== '' && newPassword !== '' && newPassword2 !== '') {
		if (newPassword.length < 6 || newPassword.length > 64 || !newPassword.match("^[^;\"']{6,64}$")) {
